...
- 分享：四页面均声明 `onShareAppMessage`（发送给朋友）与 `onShareTimeline`（分享到朋友圈）；价格详情页经 `_shareId` 在 path/query 携带记录 `id`；未设自定义分享图（默认页面截图）。
...
- 2026-09 — 四页面新增分享功能（发送给朋友 + 分享到朋友圈），README 增加「分享功能」章节。
