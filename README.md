...
四个页面均声明了 `onShareAppMessage`（发送给朋友）与 `onShareTimeline`（分享到朋友圈），声明后微信右上角「…」菜单自动出现对应入口：

| 页面 | 分享标题 | 朋友路径 / 朋友圈 query |
| --- | --- | --- |
| 比一比 | 一秤清欢·比一比｜买菜之前，先算清楚 | `/pages/compare/index` |
| 菜场指南 | 一秤清欢·菜场指南｜近期菜价参考 | `/pages/market-guide/index` |
| 价格详情 | 「{商品名}」近期参考价｜一秤清欢（加载完成前为「一秤清欢·价格详情」） | 携带 `id` 参数，接收方打开同一条价格详情 |
| 我的 | 一秤清欢｜买菜之前，先算清楚 | `/pages/mine/index` |

说明：

- 未设置自定义分享图（遵循零资源原则），微信默认截取当前页面截图作为分享配图；
- 朋友圈分享为微信「单页模式」：接收方仅打开被分享页面，价格详情页经 `query` 中的 `id` 还原记录；
- 分享标题均为中性表述，符合项目文案红线（不含煽动性表述）。
